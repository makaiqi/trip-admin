    ...
    <div class="base-block-right">
      <el-card>
        ...
        <el-form label-position="top" ref="formAddOrUpdate" :model="formAddOrUpdate" :rules="ruleAddOrUpdate" label-width="80px">
          ...
          <div style="color:red">产品类别，产品实例，标签三选一填写</div>
          <el-form-item label="产品类别">
            <zzu-product-select v-model="formAddOrUpdate.tTypeId" placeholder="产品类别" :disabled="disabledOne"></zzu-product-select>
          </el-form-item>
          <el-form-item label="产品实例">
            <zzu-product-example v-model="formAddOrUpdate.eccProductId" placeholder="产品实例" :disabled="disabledTwo"></zzu-product-example>
          </el-form-item>
          <el-form-item label="标签">
            <zzu-tag-list v-model="tagList" :max="1" tag-type="store" name="tagName" placeholder="标签" :disabled="disabledThree"></zzu-tag-list>
          </el-form-item>
...
<script type="text/babel">
  ...
  import zzuProductSelect from '@/components/product-select'
  import zzuProductExample from '@/components/product-example'

  export default {
    name: "wxMenuList",
    components: {
      zzuUdc, zzuCitySelect, zzuVendorSelect, zzuAdTypeSelect, zzuUpload, zzuTagList, zzuMenuIconSelect,zzuProductSelect,zzuProductExample
    },
    data() {
      return {
        disabledOne: false,
    ...
    watch: {
      ...
      'formAddOrUpdate.eccProductId' (newVal,oldVal) {
        if (!newVal) {
        ...
        if (newValue.length > 0) {
          for (let obj in this.formAddOrUpdate) {
            if(obj == 'eccProductId') {
              ...
              if(newValue[0][obj]) {
                this.formAddOrUpdate[obj] = newValue[0][obj]
                this.tagList = newValue[0].tagDomainList
              } else {
                continue
              }
            }
      ...
      "formAddOrUpdate.tTypeId" (newValue, oldValue) {
        if (!newValue) {
          this.disabledOne = false
          this.disabledTwo = false
          this.disabledThree = false
    ...
    methods: {
      ...
      saveOrUpdate (formAddOrUpdate) {
        if (formAddOrUpdate.eccProductId == '' && formAddOrUpdate.tTypeId == '' && formAddOrUpdate.tTagId == '') {
          this.$message({
            message: "产品类别，产品实例，标签三选一填写！",
            type: "warning"
          })
          return
        }
      ...
      resetForm() {
        for (let obj in this.formAddOrUpdate) {
          ...
          this.formAddOrUpdate.typeCode = 'V002'
        }
      },
      deleteData() {
        let query = [];
