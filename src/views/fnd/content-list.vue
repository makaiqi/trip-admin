            ...
            <template v-for="item in tableList.columns">
              <el-table-column v-if="item.key == 'assetDomain'" :type="item.type" :fixed="item.fixed" :width="item.width || 150" :label="item.title" :sortable="item.sortable" :prop="item.key">
                <template slot-scope="scope">
                  <div v-if ="scope.row.assetDomain && scope.row.assetDomain.url"  style="width: 50px;height: 50px;border: 1px solid #ddd;padding: 4px;">
                    <img :src="scope.row.assetDomain.url" alt="" style="width: 100%;height: 100%;"/>
                  </div>
                </template>
              </el-table-column>
              <el-table-column v-else-if="item.key == 'isOnline'" :fixed="item.fixed" :type="item.type" :width="item.width || 150" :label="item.title" :sortable="item.sortable" :prop="item.key">
                <template slot-scope="scope">
                  <span>{{ scope.row.isOnline ? '是' : '否'}}</span>
                </template>
              </el-table-column>
              <el-table-column v-else-if="item.key == 'isStrategy'" :fixed="item.fixed" :type="item.type" :width="item.width || 150" :label="item.title" :sortable="item.sortable" :prop="item.key">
                <template slot-scope="scope">
...
<script type="text/babel">
  ...
  export default {
    ...
    methods: {
      findPage() {
        this.$http.get(`biz/Content/list`,
          {
