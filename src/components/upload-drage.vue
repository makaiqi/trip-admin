...
<template>
  <div>
    ...
    <div class="color">
      <div
        class="color-item"
        v-for="(img,index) in currentValue" v-dragging="{ item: img, list: currentValue, group: 'img' }"
        :key="img.url"
      >
        <img  class="color-item-img" :src="img.url" alt=""  @click="imgVisable(img.url)" @dblclick="delItem(index)">
      </div>
    </div>
    ...
    methods: {
      ...
      beforeUploadHandler(file) {
        ...
        return new Promise((resolve, reject) => {
          this.$http.post('test/upToken').then(rs => {
            if (rs.data.code == 0) {
